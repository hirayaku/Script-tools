...
It's necessary to change ssa file in order to modify
font type, size, color (primary and secondary), color (outline 
and background), style (bold and italic), shadow, etc.
ssa file format itself is simplistic enough to edit with a text
editor, it is not convenient for batch editing.
This script tool in python is designed to solve that. Eventually,
this file will be packaged as a python module, and able to be
used as a script as well.

Note that this parser can be used to parsing general structured
files with organization like:

[Section xxx]
entry name: haha
entry name: hehe

When colons are replaced with '=', the format is the same as
.desktop file widely used in gnome desktop environment and other
linux utilities.
'''
import sys, os
import argparse
